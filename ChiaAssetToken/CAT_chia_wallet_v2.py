...
class WalletTool:
    ...
    async def  push_transaction(self):           
        ...
        self.CAT_ASSET_ID = "73f33751aa1bdb798adf5fec51cb9006105972576e73bbd002f2b1b243553aef"
        ...
        CHIVES_CAT_KITTY_AIRDROP_COINBASE = r.hgetall("CHIVES_CAT_KITTY_AIRDROP_COINBASE")
        ADDRESS_NUMBER = 0
        SEND_ADDRESS_KEY_CACHE = []
        SendToAmountArray = []
        SendToPuzzleHashArray = []
        SendToMemosArray = []
        for SEND_ADDRESS_KEY,SEND_ADDRESS_VALUE in CHIVES_CAT_KITTY_AIRDROP_COINBASE.items():
            if int(SEND_ADDRESS_VALUE)==0:
                ...
                SendToAmount = 100*1000
                #SendToAddress = str(SEND_ADDRESS_KEY)
                #SendToAddress = str("xcc1rwdwhhlgerrl7p5n8g4gzc0kugcdvrpvmdj9yetpuvtekrhsgaws8hunx9")
                ...
                SendToPuzzleHashArray.append(hexstr_to_bytes(SEND_ADDRESS_KEY))
                SendToMemosArray.append("")
                if ADDRESS_NUMBER>300:
                    break;
        #print(SendToAmountArray)
        #print(SendToPuzzleHashArray)
        #return;
        #SendToAmountArray.append(SendToAmount)
        #SendToPuzzleHashArray.append(hexstr_to_bytes("7fcacb05368f6612c1cef7797a594956428346addf8e55217684102d3bc7593c"))
        #SendToMemosArray.append("")
        ...
        chia_CAT_COIN_NAME_IS_USED_ARRAY = []
        for row in rows:
            coin = Coin(bytes32(bytes.fromhex(row[6])), bytes32(bytes.fromhex(row[5])), uint64.from_bytes(row[7]))
            #查询该COIN是否被花费
            if r.hget("chia_CAT_COIN_NAME_IS_USED",str(coin.name())) is None:
                ...
                chia_CAT_COIN_NAME_IS_USED_ARRAY.append(str(coin.name()))
        ...
        if push_tx['status']=="SUCCESS" and push_tx['success']==True:
            ...
            for SEND_ADDRESS_KEY_CACHE_KEY in SEND_ADDRESS_KEY_CACHE:
                r.hset("CHIVES_CAT_KITTY_AIRDROP_COINBASE",SEND_ADDRESS_KEY_CACHE_KEY,1)
            #作废已经花费过的COIN
            for chia_CAT_COIN_NAME_IS_USED_KEY in chia_CAT_COIN_NAME_IS_USED_ARRAY:
                r.hset("chia_CAT_COIN_NAME_IS_USED",chia_CAT_COIN_NAME_IS_USED_KEY,1)
        #print(type(push_tx))
